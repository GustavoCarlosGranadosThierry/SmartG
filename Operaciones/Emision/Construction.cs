...
namespace SmartG.Operaciones.Emision
{
    public partial class Construction : Form
    {
        ...
        void cargarCoberturas()
        {
            ...
            {
                liIncCoberturasDBTableAdapter.FillByTodosDBProducto(this.liabilityInc1.LiIncCoberturasDB, ConstructionLOB, Origen, productoSel);
                ...
                for (int i = 0; i < idCoberturas.Count(); i++)
                {
                    ...
                    for (int j = 0; j < dgCoberturasDB.Rows.Count; j++)
                    {
                        if (idCoberturas[i] == Convert.ToInt32(dgCoberturasDB.Rows[j].Cells["ID"].Text.ToString()))
                        {
                           ...
                           ConstructionLOB, dgCoberturasDB.Rows[j].Cells["Cobertura"].Text.ToString(), dgCoberturasDB.Rows[j].Cells["CoberturaIngles"].Text.ToString(),
                    ...
                    if (!encontro)
                    {
                        ...
                        liabilityInc1.LiIncCoberturas.Rows.Add(cobTMP.ID, ConstructionLOB, cobTMP.Cobertura, cobTMP.CoberturaIngles, cobTMP.GeniusCode, cobTMP.Defecto, cobTMP.userAdd, cobTMP.Eliminado, cobTMP.Origen);
                    }
                }
            }
        ...
        bool guardarPoliza()
        {
            try
            {
                ...
                nuevaPoliza.LineaNegocios = ConstructionLOB;
            ...
            try
            {
                ...
                for (int i = 0; i < dgCoberturas.Rows.Count; i++)
                {
                    ...
                    if (Convert.ToInt32(dgCoberturas.Rows[i].Cells["ID"].Text.ToString()) < 0)
                    {
                        ...
                        nuevaCoberturaDB.LineaNegocios = ConstructionLOB;
...
+ Environment.NewLine + x.Municipio + " " + x.Estado + Environment.NewLine + "CP " + x.CP + ", " + x.Pai.Nombre).SingleOrDefault();
        ...
        void iniciarDatos()
        {
            ...
            productoTableAdapter.FillByProductosActivos(this.liabilityInc1.Producto, ConstructionLOB, Origen);
            ...
            liIncProgramasTableAdapter.FillByDefaultLiInc(this.liabilityInc1.LiIncProgramas, ConstructionLOB, Origen);
            // llena las monedas default
            liIncMonedaTableAdapter.Fill(this.liabilityInc1.LiIncMoneda);
            cbMoneda.Value = 1;
            // llena los producing office default
            lNPOTableAdapter.FillByConsultaLNPOporIDLineaNegocio(this.liabilityInc1.LNPO, ConstructionLOB);
            cbProducingOffice.DisplayMember = "Producing Office";
            cbProducingOffice.ValueMember = "ID";
            // llena los activity Code
            liIncActivityCodeTableAdapter.FillByDefaultLi(this.liabilityInc1.LiIncActivityCode, ConstructionLOB);
            ...
            liIncCoberturasDBTableAdapter.FillByDefaultDBOrigenProducto(this.liabilityInc1.LiIncCoberturasDB, ConstructionLOB, Origen, productoSel);
            // llenado de las coberturas default
            liIncCoberturasTableAdapter.FillByDefaultOrigenProducto(this.liabilityInc1.LiIncCoberturas, ConstructionLOB, Origen, productoSel);
            dgCoberturas.DisplayLayout.PerformAutoResizeColumns(false, Infragistics.Win.UltraWinGrid.PerformAutoSizeType.AllRowsInBand);
            dgCoberturasDB.DisplayLayout.PerformAutoResizeColumns(false, Infragistics.Win.UltraWinGrid.PerformAutoSizeType.AllRowsInBand);
            ...
            DataTable dttmpEnd = endosoEmisionTableAdapter.GetDataByActivos(ConstructionLOB, Origen);
        ...
        {
            dbSmartGDataContext db = new dbSmartGDataContext();
            int[] coberturaTmp = (from x in db.Coberturas where x.LineaNegocios == ConstructionLOB && x.Producto == productoSel && x.Origen == Origen select x.ID).ToArray();
            string tmpTextos = "";
        ...
        void recargarCatalogos()
        {
            ...
            liIncFormaPagoTableAdapter.Fill(this.liabilityInc1.LiIncFormaPago);
            // llenado de las coberturas DB
            liIncCoberturasDBTableAdapter.FillByDefaultDBOrigen(this.liabilityInc1.LiIncCoberturasDB, ConstructionLOB, Origen);
            // llenado de las coberturas default
            liIncCoberturasTableAdapter.FillByDefaultOrigen(this.liabilityInc1.LiIncCoberturas, ConstructionLOB, Origen);
            dgCoberturas.DisplayLayout.PerformAutoResizeColumns(false, Infragistics.Win.UltraWinGrid.PerformAutoSizeType.AllRowsInBand);
            dgCoberturasDB.DisplayLayout.PerformAutoResizeColumns(false, Infragistics.Win.UltraWinGrid.PerformAutoSizeType.AllRowsInBand);
            // busqueda del coaseguro default xl seguros
            idDefaultCoaseguradora = liIncCoaseguradorasTableAdapter.ScalarIDCoaseguradoraXL("XL Seguros México, S.A. de C.V.");
        ...
        bool validarCorrectos()
        {
            for (int i = 0; i < 9; i++)
            {
                if (tabControlConstruction.Tabs[i].Appearance.ForeColor != Color.Green)
                {
                    return false;
                }
            }
        ...
        void validarDatos(int indiceTab)
        {
            ...
            if (tmpValida)
            {
                tabControlConstruction.Tabs[indiceTab].Appearance.BorderColor3DBase = Color.Green;
                tabControlConstruction.Tabs[indiceTab].Appearance.ForeColor = Color.Green;
            }
            else
            {
                tabControlConstruction.Tabs[indiceTab].Appearance.BorderColor3DBase = Color.Red;
                tabControlConstruction.Tabs[indiceTab].Appearance.ForeColor = Color.Red;
            }
        ...
        private void btnEnviarCobertura_Click(object sender, EventArgs e)
        {
            if (dgCoberturasDB.Selected.Rows.Count == 1)
            {
                liabilityInc1.LiIncCoberturas.Rows.Add(Convert.ToInt32(dgCoberturasDB.ActiveRow.Cells["ID"].Text.ToString()),
                    ConstructionLOB, dgCoberturasDB.ActiveRow.Cells["Cobertura"].Text.ToString(), dgCoberturasDB.ActiveRow.Cells["CoberturaIngles"].Text.ToString(),
        ...
        private void btnQuitarCobertura_Click(object sender, EventArgs e)
        {
            if (dgCoberturas.Selected.Rows.Count == 1)
            {
                liabilityInc1.LiIncCoberturasDB.Rows.Add(Convert.ToInt32(dgCoberturas.ActiveRow.Cells["ID"].Text.ToString()),
                   ConstructionLOB, dgCoberturas.ActiveRow.Cells["Cobertura"].Text.ToString(), dgCoberturas.ActiveRow.Cells["CoberturaIngles"].Text.ToString(),
        ...
        public Construction(int origen, int idVentana = 0, int idPolizaTemp = 0)
        {
            InitializeComponent();
            ...
            ConstructionLOB = (from x in db.LineaNegocios where x.LineaNegocios1 == "Construction" select x.ID).SingleOrDefault();
            Origen = origen;
            ventana = idVentana;
            if (idPolizaTemp != 0)
        ...
        private void Construction_FormClosing(object sender, FormClosingEventArgs e)
        {
            if (controlSave != true)
            {
        ...
        private void Construction_Load(object sender, EventArgs e)
        {
            ...
            validarDatos(tabControlConstruction.ActiveTab.Index);
            txtRetroValidaciones.Text = "";
            tabAnterior = tabControlConstruction.ActiveTab.Index;
            this.FormClosing += Construction_FormClosing;
        }
        ...
        private void ToolsBarConstruction_ToolClick(object sender, Infragistics.Win.UltraWinToolbars.ToolClickEventArgs e)
        {
            terminarEdicionGrids();
            dbSmartGDataContext db = new dbSmartGDataContext();
                        ...
                        if (MessageBox.Show("Para utilizar esta función es necesario validar los datos de la póliza, ¿deseas continuar?", "Aviso", MessageBoxButtons.YesNo, MessageBoxIcon.Asterisk) == DialogResult.Yes)
                        {
                            ...
                            if (validarCorrectos() && validarCliente())
                            {
                                if (MessageBox.Show("Esta función concluirá el registro y generará los documentos finales, se recomienda generar un previo de la póliza para su revisión ya que una vez finalizado el registro no se podrá cambiar, si aun así deseas continuar con la conclusión del proceso has click en Si", "Aviso importante", MessageBoxButtons.YesNo, MessageBoxIcon.Warning) == DialogResult.Yes)
                                {
                                    ...
                                    DocumentosDB.GuardarDocumentosDB(rutaGuardado, Convert.ToInt32(idPoliza), ConstructionLOB, polizaMX, txtPAM.Text, emision);
        ...
        private void txtNuevaCobertura_KeyDown(object sender, KeyEventArgs e)
        {
            ...
            {
                liabilityInc1.LiIncCoberturas.Rows.Add(coberturaM, ConstructionLOB, txtNuevaCobertura.Text, "N/A", "OTH", false, true, false, 1);
                coberturaM--;
