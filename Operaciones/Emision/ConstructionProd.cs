...
{
    public partial class ConstructionProd : Form
    {
        //**********************************************************************************
        //**********************************************************************************
        //**********************************************************************************
        ...
        void cargarCoberturas()
        {
            ...
            {
                coberturasDBTableAdapter.FillByTodosDB(this.coberturasOrdenadas.CoberturasDB, Construccion, Origen);
                ...
                for (int i = 0; i < idCoberturas.Count(); i++)
                {
                    ...
                    for (int j = 0; j < dgCoberturasDB.Rows.Count; j++)
                    {
                        if (idCoberturas[i] == Convert.ToInt32(dgCoberturasDB.Rows[j].Cells["ID"].Text.ToString()))
                        {
                           ...
                           Construccion, dgCoberturasDB.Rows[j].Cells["Cobertura"].Text.ToString(), dgCoberturasDB.Rows[j].Cells["CoberturaIngles"].Text.ToString(),
                    ...
                    if (!encontro)
                    {
                        ...
                        coberturasOrdenadas.Coberturas.Rows.Add(cobTMP.ID, Construccion, cobTMP.Cobertura, cobTMP.CoberturaIngles, cobTMP.GeniusCode, cobTMP.Defecto, cobTMP.userAdd, cobTMP.Eliminado, cobTMP.Origen, cobTMP.OrdenImpresion);
                    }
                }
            }
            ...
            try
            {
                ...
                if (chkLimites.Checked)
                {
                    ...
                    for (int i = 0; i < dgLimites.Rows.Count; i++)
                    {
                        ...
                        nuevaPolizaLim.Monto = Convert.ToDecimal(dgLimites.Rows[i].Cells["Monto"].Text);
        ...
        bool guardarPoliza()
        {
            try
            {
                ...
                nuevaPoliza.LineaNegocios = Construccion;
            ...
            try
            {
                ...
                for (int i = 0; i < dgCoberturas.Rows.Count; i++)
                {
                    ...
                    if (Convert.ToInt32(dgCoberturas.Rows[i].Cells["ID"].Text.ToString()) < 0)
                    {
                        ...
                        nuevaCoberturaDB.LineaNegocios = Construccion;
...
+ Environment.NewLine + x.Municipio + " " + x.Estado + Environment.NewLine + "CP " + x.CP + ", " + x.Pai.Nombre).SingleOrDefault();
        ...
        void iniciarDatos()
        {
            ...
            liIncProgramasTableAdapter.FillByDefaultLiInc(this.liabilityInc1.LiIncProgramas, Construccion, Origen);
            // llena las monedas default
            liIncMonedaTableAdapter.Fill(this.liabilityInc1.LiIncMoneda);
            cbMoneda.Value = 1;
            // llena los producing office default
            lNPOTableAdapter.FillByConsultaLNPOporIDLineaNegocio(this.liabilityInc1.LNPO, Construccion);
            cbProducingOffice.DisplayMember = "Producing Office";
            cbProducingOffice.ValueMember = "ID";
            // llena los activity Code
            liIncActivityCodeTableAdapter.FillByDefaultLi(this.liabilityInc1.LiIncActivityCode, Construccion);
            ...
            coberturasDBTableAdapter.FillByDefaultDBOrigen(this.coberturasOrdenadas.CoberturasDB, Construccion, Origen);
            // llenado de las coberturas default
            coberturasTableAdapter.FillByDefaultOrigen(this.coberturasOrdenadas.Coberturas, Construccion, Origen);
            dgCoberturas.DisplayLayout.PerformAutoResizeColumns(false, Infragistics.Win.UltraWinGrid.PerformAutoSizeType.AllRowsInBand);
            dgCoberturasDB.DisplayLayout.PerformAutoResizeColumns(false, Infragistics.Win.UltraWinGrid.PerformAutoSizeType.AllRowsInBand);
            ...
            DataTable dttmpEnd = endosoEmisionTableAdapter.GetDataByActivos(Construccion, Origen);
        ...
        void iniciarLimites()
        {
            ...
            //dgLimites.DisplayLayout.Bands[0].Columns["Limite"].SortIndicator = Infragistics.Win.UltraWinGrid.SortIndicator.Ascending;
        ...
        void llenarTablaReaseguro()
        {
            if (dgReaseguro.Rows.Count == 0)
            {
                // inicializamos el grid y lo formateamos
                dgReaseguro.DataSource = dtReaseguro;
                DataTable dtReaseTMP = liIncReaseguradorasTableAdapter.GetDataByDefaultPR();
        ...
        void validarDatos(int indiceTab)
        {
            ...
            switch (indiceTab)
            {
                ...
                case 4: // tab limites
                    ...
                    for (int i = 0; i < dgLimites.Rows.Count; i++)
                    {
                        if (Convert.ToDecimal(dgLimites.Rows[i].Cells["Monto"].Text) <= 0)
                        {
                            tmpValida = false;
                            ...
                            switch (caso)
                            {
                                ...
                                case 2:
                                    if (idDefaultCoaseguradora == Convert.ToInt32(dgCoaseguro.Rows[0].Cells["Coaseguradora"].Value) && cbTipoCoaseguro.Text != "")
                                    {
                                        ...
                                        if (cbTipoCoaseguro.Text == "Coaseguro Lider")
                                            coaseguroReg = Convert.ToDouble(dgCoaseguro.Rows[0].Cells["Participacion"].Text);
                                        else
                                ...
                                case 3:
                                    ...
                                    {//Participacion
                                        double reaseguroReg = Convert.ToDouble(dgReaseguro.Rows[0].Cells["Participacion"].Text);
        ...
        private void btnEnviarCobertura_Click(object sender, EventArgs e)
        {
            if (dgCoberturasDB.Selected.Rows.Count == 1)
            {
                coberturasOrdenadas.Coberturas.Rows.Add(Convert.ToInt32(dgCoberturasDB.ActiveRow.Cells["ID"].Text.ToString()),
                    Construccion, dgCoberturasDB.ActiveRow.Cells["Cobertura"].Text.ToString(), dgCoberturasDB.ActiveRow.Cells["CoberturaIngles"].Text.ToString(),
        ...
        private void btnQuitarCobertura_Click(object sender, EventArgs e)
        {
            if (dgCoberturas.Selected.Rows.Count == 1)
            {
                coberturasOrdenadas.CoberturasDB.Rows.Add(Convert.ToInt32(dgCoberturas.ActiveRow.Cells["ID"].Text.ToString()),
                   Construccion, dgCoberturas.ActiveRow.Cells["Cobertura"].Text.ToString(), dgCoberturas.ActiveRow.Cells["CoberturaIngles"].Text.ToString(),
        ...
        public ConstructionProd(int idVentana = 0, int idPolizaTemp = 0)
        {
            InitializeComponent();
            ...
            Construccion = (from x in db.LineaNegocios where x.LineaNegocios1 == "Property" select x.ID).SingleOrDefault();
        ...
        private void ConstructionProd_FormClosing(object sender, FormClosingEventArgs e)
        {
            if (controlSave != true)
            {
        ...
        private void ConstructionProd_Load(object sender, EventArgs e)
        {
                        ...
                        if (MessageBox.Show("Para utilizar esta función es necesario validar los datos de la póliza, ¿deseas continuar?", "Aviso", MessageBoxButtons.YesNo, MessageBoxIcon.Asterisk) == DialogResult.Yes)
                        {
                            ...
                            if (validarCorrectos() && validarCliente())
                            {
                                if (MessageBox.Show("Esta función concluirá el registro y generará los documentos finales, se recomienda generar un previo de la póliza para su revisión ya que una vez finalizado el registro no se podrá cambiar, si aun así deseas continuar con la conclusión del proceso has click en Si", "Aviso importante", MessageBoxButtons.YesNo, MessageBoxIcon.Warning) == DialogResult.Yes)
                                {
                                    ...
                                    DocumentosDB.GuardarDocumentosDB(rutaGuardado, Convert.ToInt32(idPoliza), Construccion, polizaMX, txtPAM.Text, emision);
        ...
        private void txtNuevaCobertura_KeyDown(object sender, KeyEventArgs e)
        {
            ...
            {
                coberturasOrdenadas.Coberturas.Rows.Add(coberturaM, Construccion, txtNuevaCobertura.Text, "N/A", "OTH", false, true, false, 1, 11);
                coberturaM--;
