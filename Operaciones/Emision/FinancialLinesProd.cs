...
{
    public partial class FinancialLinesProd : Form
    {
        //**********************************************************************************
        //**********************************************************************************
        //**********************************************************************************
        ...
        //cbJurisdiccion		Solo Nacional, Mundial, Otro
        //lbOtro			Otro
        //txtJurisOtro
        //lbPeriodoDescubrimiento	Periodo de descubrimiento:
        ...
        bool borrarRegistros(int solicitud)
        {
            ...
            try
            {
                switch (solicitud)
                {
                    ...
                    case 4: // PolizaFLProducing
                        PolizaFLProducing aBorrarProd = (from x in db.PolizaFLProducing where x.PolizaFL == idPolizaFL select x).SingleOrDefault();
                        if (aBorrarProd != null)
                        {
                            db.PolizaFLProducing.DeleteOnSubmit(aBorrarProd);
                            db.SubmitChanges();
                        }
                        break;
                    ...
                    case 8: // endosos
                        PolizaEndosoEmision[] aBorrarEndosos = (from x in db.PolizaEndosoEmision where x.Poliza == idPoliza select x).ToArray();
                        if (aBorrarEndosos.Count() > 0)
                        {
                            db.PolizaEndosoEmision.DeleteAllOnSubmit(aBorrarEndosos);
                            db.SubmitChanges();
                        }
                        break;
                }
        ...
        void cargarCoberturas()
        {
            ...
            {
                liIncCoberturasDBTableAdapter.FillByTodosDB(this.liabilityInc1.LiIncCoberturasDB, FinancialLines, Origen);
                ...
                for (int j = 0; j < endosos.Count(); j++)
                {
                    for (int i = 0; i < dgEndosos.Rows.Count; i++)
                    {
                        if (Convert.ToInt32(dgEndosos.Rows[i].Cells["ID"].Text) == endosos[j].EndosoEmision)
                        {
                            ...
                            dgEndosos.Rows[i].Cells["Texto Agregado"].Value = endosos[j].Texto;
                        }
                    }
                }
            }
        ...
        void cargarPoliza()
        {
            ...
            PolizaFLProducing tmpPolizaFLProd = (from x in db.PolizaFLProducing where x.PolizaFL == idPolizaFL select x).SingleOrDefault();
            if (tmpPolizaFLProd != null)
            {
                txtQuoteNumber.Text = tmpPolizaFLProd.QuoteNumber;
                cbJurisdiccion.Text = tmpPolizaFLProd.Jurisdiccion;
                txtJurisOtro.Text = tmpPolizaFLProd.JurisdiccionOtro;
                try
                {
                    txtPeriodoDescubrimiento.Rtf = tmpPolizaFLProd.PeriodoDescubrimiento;
                }
                catch
                {
                    txtPeriodoDescubrimiento.Text = tmpPolizaFLProd.PeriodoDescubrimiento;
                }
            }
        ...
        void generarWording(string file, int tipo)
        {
            ...
            try
            {
                ...
                if (doc.Bookmarks.Exists(bookmarkName.ToString()))
                {
                    ...
                    {
                        tabla.Rows.Add();
                        tabla.Cell(fila, 1).Select(); ac.Selection.Font.Bold = 1; ac.Selection.TypeText("Deducibles:" + Environment.NewLine + "(Los Deducibles no son aplicables a Pérdidas de cualquier Persona Asegurada a menos que tales Pérdidas puedan ser indemnizadas por la Sociedad)");
                    ...
                    {
                        tabla.Rows.Add();
                        tabla.Cell(fila, 1).Select(); ac.Selection.Font.Bold = 1; ac.Selection.TypeText("Fecha Retroactiva:");
                        tabla.Cell(fila, 2).Select(); ac.Selection.TypeText(strRetroactiva + Environment.NewLine); fila++;
                    }
                    tabla.Rows.Add();
                    tabla.Cell(fila, 1).Select(); ac.Selection.Font.Bold = 1; ac.Selection.TypeText("Fecha de continuidad:");
                    tabla.Cell(fila, 2).Select(); ac.Selection.TypeText(strContinuidad + Environment.NewLine); fila++;
                    tabla.Rows.Add();
                    tabla.Cell(fila, 1).Select(); ac.Selection.Font.Bold = 1; ac.Selection.TypeText("Jurisdicción:");
                    tabla.Cell(fila, 2).Select(); ac.Selection.TypeText(strJurisdiccion + Environment.NewLine); fila++;
                    if (txtPeriodoDescubrimiento.Text != "")
                    {
                        bool tipoTexto = false;
                        tabla.Rows.Add();
                        tabla.Cell(fila, 1).Select(); ac.Selection.Font.Bold = 1; ac.Selection.TypeText("Periodo de descubrimiento:");
                        try
                        { Clipboard.SetText(txtPeriodoDescubrimiento.Rtf, TextDataFormat.Rtf); tipoTexto = true; }
                        catch
                        { Clipboard.SetText(txtPeriodoDescubrimiento.Text, TextDataFormat.Text); tipoTexto = false; }
                        if (tipoTexto)
                ...
                #region Remplazar Fechas Endosos
                ...
                encuentra.Text = "[FechaEndoso]";
                encuentra.Replacement.ClearFormatting();
                encuentra.Replacement.Text = Convert.ToDateTime(dateInicioVig.Value).ToShortTimeString() + " del " + formatearFecha(Convert.ToDateTime(dateInicioVig.Value), 2);
                object remplaza = Word.Enums.WdReplace.wdReplaceAll;

                encuentra.Execute(m, m, m, m, m, m, m, m, m, m, remplaza, m, m, m, m);
                #endregion

                #region Remplazar textos para endosos
                for (int i = 0; i < dgEndosos.Rows.Count; i++)
                {
                    if (Convert.ToBoolean(dgEndosos.Rows[i].Cells["Aplica"].Value))
                    {
                        object inicioE = doc.Content.Start;
                        object finE = doc.Content.End;
                        Word.Range rngE = doc.Range(inicioE, finE);
                        rngE.Select();

                        Word.Find encuentraE = ac.Selection.Find;
                        encuentraE.Text = "[" + dgEndosos.Rows[i].Cells["Endoso"].Text + "]";
                        encuentraE.Replacement.ClearFormatting();
                        encuentraE.Replacement.Text = dgEndosos.Rows[i].Cells["Texto Agregado"].Text;
                        object remplazaE = Word.Enums.WdReplace.wdReplaceAll;

                        encuentraE.Execute(m, m, m, m, m, m, m, m, m, m, remplazaE, m, m, m, m);
                    }
                }
                #endregion
            ...
            {
                txtRetroValidaciones.Text += Environment.NewLine + "Ocurrió un error al generar el wording, reintentando generar el documento.";
                ((Word._Document)doc).Close();
                ((Word._Application)ac).Quit();
                errorCount++;
                if (errorCount == 1 && tipo == 1)
                {
                    generarWording(file, 1);
                    //MessageBox.Show("Ocurrió un error al generar el wording, reintentando generar el documento." + Environment.NewLine + ex.ToString(), "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                }
                else if (errorCount == 1 && tipo == 2)
                {
                    generarWording(file, 2);
                   //MessageBox.Show("Ocurrió un error al generar el wording, reintentando generar el documento." + Environment.NewLine + ex.ToString(), "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                }
                else
                    File.Delete("C:\\SmartG\\" + file); // borramos el documento temporal
            ...
            try
            {
                ...
                if (chkDeducibles.Checked) // Se salta todo el proceso si No hay deducibles
                {
                    for (int i = 0; i < dgDeducibles.Rows.Count; i++)
                    {
                        ...
                        string maxDeduc = double.Parse(dgDeducibles.Rows[i].Cells["Maximo"].Text.ToString()).ToString("#,##0.00", new CultureInfo("en-US"));
                        string minDeduc = double.Parse(dgDeducibles.Rows[i].Cells["Minimo"].Text.ToString()).ToString("#,##0.00", new CultureInfo("en-US"));
                        string aggregationDeduc = double.Parse(dgDeducibles.Rows[i].Cells["Agregado"].Text.ToString()).ToString("#,##0.00", new CultureInfo("en-US"));
                        string percentajeDeduc = double.Parse(dgDeducibles.Rows[i].Cells["Porcentaje"].Text.ToString()).ToString("#,##0.00", new CultureInfo("en-US")) + " %";
                    ...
                    case "Coaseguro Lider":
                        ...
                        for (int i = 0; i < dgCoaseguro.Rows.Count; i++)
                        {
                            if (dgCoaseguro.Rows[i].Cells["Coaseguradora"].Text != "")
                            {
                                ...
                                minorParticipationLine = double.Parse(dgCoaseguro.Rows[i].Cells["PorcentajeParticipacion"].Text).ToString("#,##0.00", new CultureInfo("en-US"));
                ...
                else // Caso con Reaseguro
                {
                    ...
                    for (int i = 0; i < dgReaseguro.Rows.Count; i++)
                    {
                        ...
                        if (i == 0) // caso para la reaseguradora por defecto
                        {
                            for (int j = 1; j < dgReaseguro.Rows.Count; j++)
                            {
                                ...
                                {
                                    RIPREMper += Convert.ToDouble(dgReaseguro.Rows[j].Cells["PorcentajeParticipacion"].Text);
                                    RICcur += Convert.ToDouble(dgReaseguro.Rows[j].Cells["Comision"].Text);
                                }
                            }
                            RIPREMcur = (RIPREMper / 100) * double.Parse(lbPrimaReaseguro.Text);
                        ...
                        {
                            RIPREMper = Convert.ToDouble(dgReaseguro.Rows[i].Cells["PorcentajeParticipacion"].Text); // % participacion
                            RIPREMcur = Convert.ToDouble(dgReaseguro.Rows[i].Cells["Participacion"].Text); // $ participacion
                            RICper = Convert.ToDouble(dgReaseguro.Rows[i].Cells["PorcentajeComision"].Text) / 100; // % comision
                            RICcur = Convert.ToDouble(dgReaseguro.Rows[i].Cells["Comision"].Text); // $ comision
                            netCession += -RIPREMcur + RICcur;
                    ...
                    double netPrima = double.Parse(lbPrimaReaseguro.Text) + (double.Parse(lbPrimaReaseguro.Text) * iva_per);

                    double netBroker = (Convert.ToDouble(txtBrokeragePorc.Value) / 100) * double.Parse(lbPrimaReaseguro.Text) * (1 + iva_per);
                ...
                else
                {
                    ...
                    double DedRIC = 0;
                    int count_part = 0;
                    DedRIC = Convert.ToDouble(dgReaseguro.Rows[0].Cells["PorcentajeComision"].Text); count_part++;
                    ...
                    for (int i = 1; i < dgReaseguro.Rows.Count; i++)
                    {
                        ...
                        {
                            partRestan += Convert.ToDouble(dgReaseguro.Rows[i].Cells["PorcentajeParticipacion"].Text) / 100;
                        }
                    }
                    ...
                    for (int i = 1; i < dgReaseguro.Rows.Count; i++)
                    {
                        ...
                        if (Convert.ToBoolean(dgReaseguro.Rows[i].Cells["Treaty"].Value) && dgReaseguro.Rows[i].Cells["Reaseguradora"].Text != "")
                        {
                            double PREM_ce = (Convert.ToDouble(dgReaseguro.Rows[i].Cells["PorcentajeParticipacion"].Text) / 100) / partRestan; // porc partici
                            double PREM_cu = -Convert.ToDouble(dgReaseguro.Rows[i].Cells["Participacion"].Text); // participacion
                            double RIC_ce = Convert.ToDouble(dgReaseguro.Rows[i].Cells["PorcentajeComision"].Text) / 100; // porc comision
                            double RIC_cu = Convert.ToDouble(dgReaseguro.Rows[i].Cells["Comision"].Text); // comision
                    ...
                    for (int i = 1; i < dgReaseguro.Rows.Count; i++)
                    {
                        if (Convert.ToBoolean(dgReaseguro.Rows[i].Cells["Treaty"].Value) && dgReaseguro.Rows[i].Cells["Reaseguradora"].Text != "")
                        {
                            SPPRE += Convert.ToDouble(dgReaseguro.Rows[i].Cells["Participacion"].Text);
                            SPRIC += Convert.ToDouble(dgReaseguro.Rows[i].Cells["Comision"].Text);
                        }
                    }
            ...
            try
            {
                ...
                for (int i = 0; i < dgEndosos.Rows.Count; i++)
                {
                    if (Convert.ToBoolean(dgEndosos.Rows[i].Cells["Aplica"].Value))
                    {
                        ...
                        nuevaPolizaEndoso.Texto = dgEndosos.Rows[i].Cells["Texto Agregado"].Text;
        ...
        bool guardarPolizaFL()
        {
            try
            {
                ...
                nuevaPolizaFL.Producto = producto;
                ...
                // borramos el registro en la tabla de FL Producing
                borrarRegistros(4);

                PolizaFLProducing nuevaPolizaFLProd = new PolizaFLProducing();
                nuevaPolizaFLProd.PolizaFL = idPolizaFL;
                nuevaPolizaFLProd.QuoteNumber = quoteNumber;
                nuevaPolizaFLProd.PeriodoDescubrimiento = periodoDescubrimiento;
                nuevaPolizaFLProd.Jurisdiccion = Jurisdiccion;
                nuevaPolizaFLProd.JurisdiccionOtro = JurisdiccionOtro;
                db.PolizaFLProducing.InsertOnSubmit(nuevaPolizaFLProd);
                db.SubmitChanges();
                return true;
            }
            catch
        ...
        void guardarVariables()
        {
            ...
            if (cbProducto.Value != null)
                producto = Convert.ToInt32(cbProducto.Value);

            Jurisdiccion = cbJurisdiccion.Text;
            JurisdiccionOtro = txtJurisOtro.Text;
            try
            {
                periodoDescubrimiento = txtPeriodoDescubrimiento.Rtf;
            }
            catch
            {
                periodoDescubrimiento = txtPeriodoDescubrimiento.Text;
            }
...
+ Environment.NewLine + x.Municipio + " " + x.Estado + Environment.NewLine + "CP " + x.CP + ", " + x.Pai.Nombre).SingleOrDefault();
            ...
            if (!txtJurisOtro.Visible)
                strJurisdiccion = cbJurisdiccion.Text;
            else
                strJurisdiccion = txtJurisOtro.Text;

            strLimite = strAbreMon + limiteMaximo.ToString("#,##0", new CultureInfo("en-US"));
            if (excesoConsejeros > 0)
            {
                strExceso = "\n Límite de exceso especial para consejeros independientes (Cobertura 2.2): " + strAbreMon + " " + excesoConsejeros.ToString("#,##0.00", new CultureInfo("en-US")) + " para toda y cada perdida y en el agregado anual \n";
            }
        ...
        void iniciarDatos()
        {
            ...
            extensionesDefault = new string[16];
            extensionesDefault[0] = "Investigación";
            extensionesDefault[1] = "Evento Regulatorio Crítico";
            extensionesDefault[2] = "Extradición";
            extensionesDefault[3] = "Bloqueo de bienes y privación de libertad";
            extensionesDefault[4] = "Costo de Peritos";
            extensionesDefault[5] = "Daño a la reputación";
            extensionesDefault[6] = "Territorio Internacional";
            extensionesDefault[7] = "Infracción de Prácticas Laborales";
            extensionesDefault[8] = "Crisis";
            extensionesDefault[9] = "Nuevas Subsidiarias";
            extensionesDefault[10] = "Período de descubrimiento";
            extensionesDefault[11] = "Período de Descubrimiento vitalicio para personas retiradas";
            extensionesDefault[12] = "Costos de emergencia";
            extensionesDefault[13] = "Lesiones corporales y daños materiales";
            extensionesDefault[14] = "Homicidio corporativo culposo (corporate manslaughter)";
            extensionesDefault[15] = "Costos de defensa y Pérdida  por daño ambiental según la condición 4.4 literales (ii) y (iii)";
            #endregion
        }
        ...
        void llenarControlesObligatorios()
        {
            ...
            controlesObligatorios[15] = txtLimiteMaximo;
            ...
            switch (indiceTab)
            {
                case 0: // tab datos generales
                    ...
                    else if (txtQuoteNumber.Text != "")
                    {
                        ...
                        {
                            tmpValida = false;
                            txtRetroValidaciones.Text += Environment.NewLine + "Error: el campo Quote Number debe llenarse correctamente: QMX + 7 dígitos seguimiento + DO + 2 dígitos año de emisión + caracter A,B o C (Datos Generales)";
                        }
                    }
                    for (int i = 2; i < 15; i++)
                    {
                    ...
                    if (txtJurisOtro.Visible && txtJurisOtro.Text == "")
                    {
                        tmpValida = false;
                        txtRetroValidaciones.Text += Environment.NewLine + "Error: debes de ingresar la zona de jurisdicción manualmente si seleccionaste la opción 'Otro' (Datos Generales)";
                    }
                ...
                case 2: // tab endosos
                    if (tmpValida) // pasó todas las validaciones
                        txtRetroValidaciones.Text += Environment.NewLine + "5) Sección Endosos OK";
                    break;
                ...
                case 3: // tab limites y sublimites
                    ...
                    #endregion
                    if (tmpValida) // pasó todas las validaciones
                        txtRetroValidaciones.Text += Environment.NewLine + "3) Sección Limites y Sublimites OK";
                    break;
                ...
                case 4: // tab deducibles y exclusiones
                    ...
                    if (tmpValida) // pasó todas las validaciones
                        txtRetroValidaciones.Text += Environment.NewLine + "4) Sección Deducibles OK";
                    break;
        ...
        {
            int? idTmp = Convert.ToInt32(dgEndosos.ActiveRow.Cells["ID"].Value.ToString());
            if (idTmp != null)
            {
        ...
        private void cbProducto_ValueChanged(object sender, EventArgs e)
        {
            ...
            //    txtQuoteNumber.InputMask = "QMX#######DO##A";
            //}
            //else
            //{
            ...
            //    txtQuoteNumber.InputMask = "QMX#######EO##A";
            //}
        ...
        private void Execute_Exited(object sender, EventArgs e)
        {
            ...
            if (proceso.ExitCode != 0)
            {
                ...
                dbSmartGDataContext db = new dbSmartGDataContext();
                EndosoEmision updateEndoso = (from x in db.EndosoEmision where x.ID == idTmp select x).SingleOrDefault();
                updateEndoso.EndosoTXT = clipboardGetData;
                db.SubmitChanges();
                MessageBox.Show("Actualizado");
            }
        }

        public FinancialLinesProd(int idVentana = 0, int idPolizaTemp = 0)
        {
            InitializeComponent();
        ...
        private void FinancialLinesProd_FormClosing(object sender, FormClosingEventArgs e)
        {
            if (controlSave != true)
            {
        ...
        private void FinancialLinesProd_Load(object sender, EventArgs e)
        {
            Extensiones.Traduccion.traducirVentana(this, tabControlLiability, ToolsBarFinancialLinesProd);
            ...
            this.FormClosing += FinancialLinesProd_FormClosing;
        }
                        ...
                        if (MessageBox.Show("Para utilizar esta función es necesario validar los datos de la póliza, ¿deseas continuar?", "Aviso", MessageBoxButtons.YesNo, MessageBoxIcon.Asterisk) == DialogResult.Yes)
                        {
                            ...
                            if (validarCorrectos() && validarCliente())
                            {
                                if (MessageBox.Show("Esta función concluirá el registro y generará los documentos finales, se recomienda generar un previo de la póliza para su revisión ya que una vez finalizado el registro no se podrá cambiar, si aun así deseas continuar con la conclusión del proceso has click en Si", "Aviso importante", MessageBoxButtons.YesNo, MessageBoxIcon.Warning) == DialogResult.Yes)
                                {
                                    ...
                                    if (nuevoPreview.ExtraerDocumentoDB("WordingFLProd.docx"))
                                    {
                                        txtRetroValidaciones.Text += Environment.NewLine + "Generando Wording...";
                                        generarWording("WordingFLProd.docx", 2);
                                    }
                                    else
                                    {
                    ...
                    if (primerSaveP)
                    {
                        if (MessageBox.Show("Para utilizar esta función es necesario validar los datos de la póliza, ¿deseas continuar?", "Aviso", MessageBoxButtons.YesNo, MessageBoxIcon.Asterisk) == DialogResult.Yes)
                        {
                            ...
                            if (validarCorrectos())
                            {
                                ...
                                if (nuevoPreview.ExtraerDocumentoDB("PreviewWordingFLProd.docx"))
                                {
                                    txtRetroValidaciones.Text += Environment.NewLine + "Generando Wording...";
                                    generarWording("PreviewWordingFLProd.docx", 1);
                                }
                                else
                                {
                                    MessageBox.Show("Ocurrió un error inesperado al generar el documento (wording), favor de contactar al soporte técnico para futura referencia", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
        ...
        {
            MessageBox.Show("El formato correcto para el Quote Number es el siguiente: QMX + 7 dígitos de seguimiento + DO + 2 dítigos del año de emisión + 1 caracter, verifica los datos ingresados", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
        }
        ...
        private void dgAseguAdicionales_AfterCellUpdate(object sender, Infragistics.Win.UltraWinGrid.CellEventArgs e)
        {
            dgAseguAdicionales.DisplayLayout.PerformAutoResizeColumns(false, Infragistics.Win.UltraWinGrid.PerformAutoSizeType.AllRowsInBand);
        }
